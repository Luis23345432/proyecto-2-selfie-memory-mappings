...
// POSIX I/O helpers for the host
uint64_t pread(int fd, void* buffer, uint64_t bytes_to_read, uint64_t offset);
uint64_t pwrite(int fd, void* buffer, uint64_t bytes_to_write, uint64_t offset);
...
uint64_t MAPPING_ROUNDED_LENGTH = 3;
uint64_t MAPPING_PROT           = 4;
uint64_t MAPPING_FD             = 5;
...
uint64_t PAGECACHE_CACHE_FRAME = 3;
uint64_t PAGECACHE_DIRTY       = 4;
uint64_t PAGECACHE_REFCOUNT    = 5;
uint64_t PAGECACHE_FD          = 6;
...
// 14 uint64_t + 6 uint64_t* + 1 char* + 7 uint64_t + 2 uint64_t* + 2 uint64_t entries + 1 uint64_t* (mappings)
...
uint64_t mappings_head(uint64_t* context) { return (uint64_t) (context + 38); }
...
uint64_t* get_mappings_head(uint64_t* context) { return (uint64_t*) *(context + 38); }
...
void set_mappings_head(uint64_t* context, uint64_t* head) { *(context + 38) = (uint64_t) head; }
...
uint64_t  get_mapping_rounded_length(uint64_t* mapping)  { return             *(mapping + MAPPING_ROUNDED_LENGTH); }
uint64_t  get_mapping_prot(uint64_t* mapping)           { return             *(mapping + MAPPING_PROT); }
uint64_t  get_mapping_fd(uint64_t* mapping)             { return             *(mapping + MAPPING_FD); }
uint64_t  get_mapping_offset(uint64_t* mapping)         { return             *(mapping + MAPPING_FILE_OFFSET); }
uint64_t  get_mapping_flags(uint64_t* mapping)          { return             *(mapping + MAPPING_FLAGS); }

void set_mapping_next(uint64_t* mapping, uint64_t* next)           { *(mapping + MAPPING_NEXT) = (uint64_t) next; }
void set_mapping_addr(uint64_t* mapping, uint64_t addr)            { *(mapping + MAPPING_ADDR) = addr; }
void set_mapping_original_length(uint64_t* mapping, uint64_t len)  { *(mapping + MAPPING_ORIG_LENGTH) = len; }
void set_mapping_rounded_length(uint64_t* mapping, uint64_t len)   { *(mapping + MAPPING_ROUNDED_LENGTH) = len; }
void set_mapping_prot(uint64_t* mapping, uint64_t prot)            { *(mapping + MAPPING_PROT) = prot; }
void set_mapping_fd(uint64_t* mapping, uint64_t fd)                { *(mapping + MAPPING_FD) = fd; }
...
uint64_t* get_page_cache_frame(uint64_t* entry)         { return (uint64_t*) *(entry + PAGECACHE_CACHE_FRAME); }
uint64_t  get_page_cache_dirty(uint64_t* entry)         { return             *(entry + PAGECACHE_DIRTY); }
uint64_t  get_page_cache_refcount(uint64_t* entry)      { return             *(entry + PAGECACHE_REFCOUNT); }
uint64_t  get_page_cache_fd(uint64_t* entry)            { return             *(entry + PAGECACHE_FD); }
uint64_t  get_page_cache_last_used(uint64_t* entry)     { return             *(entry + PAGECACHE_LAST_USED); }

void set_page_cache_next(uint64_t* entry, uint64_t* next)      { *(entry + PAGECACHE_NEXT) = (uint64_t) next; }
void set_page_cache_file_id(uint64_t* entry, uint64_t file_id) { *(entry + PAGECACHE_FILE_ID) = file_id; }
void set_page_cache_file_page(uint64_t* entry, uint64_t page)  { *(entry + PAGECACHE_FILE_PAGE) = page; }
void set_page_cache_frame(uint64_t* entry, uint64_t* frame)    { *(entry + PAGECACHE_CACHE_FRAME) = (uint64_t) frame; }
void set_page_cache_dirty(uint64_t* entry, uint64_t dirty)     { *(entry + PAGECACHE_DIRTY) = dirty; }
void set_page_cache_refcount(uint64_t* entry, uint64_t refcount) { *(entry + PAGECACHE_REFCOUNT) = refcount; }
...
uint64_t get_file_id_for_fd(uint64_t fd);
void touch_page_cache_entry(uint64_t* entry);
uint64_t* allocate_cache_frame();
uint64_t* evict_page_cache_entry();
uint64_t* find_page_cache_entry(uint64_t fd, uint64_t file_page);
uint64_t* find_or_create_cache_entry(uint64_t fd, uint64_t file_page);
void increment_page_cache_refcount(uint64_t* entry);
...
uint64_t  is_address_in_mapping(uint64_t* mapping, uint64_t vaddr);
uint64_t  do_ranges_overlap(uint64_t start1, uint64_t len1, uint64_t start2, uint64_t len2);
uint64_t  is_mapping_prot_valid(uint64_t prot);
uint64_t  is_mapping_readable(uint64_t prot);
uint64_t  is_mapping_writable(uint64_t prot);
uint64_t  is_mmap_range_free(uint64_t* context, uint64_t addr, uint64_t length);
uint64_t  choose_mmap_address(uint64_t* context, uint64_t length);
...
uint64_t is_valid_mapping_write(uint64_t vaddr);
...
void emit_mmap() {
	...
	// return value in REG_A0 (mapped address)
	emit_jalr(REG_ZR, REG_RA, 0);
}
...
void emit_munmap() {
	...
	// return value in REG_A0 (0 on success, -1 on error)
	emit_jalr(REG_ZR, REG_RA, 0);
}
...
uint64_t is_valid_segment_read(uint64_t vaddr) {
  if (is_data_address(current_context, vaddr)) {
    ...
    return 1;
  } else if (is_valid_mapping_read(vaddr))
    return 1;
  else
    return 0;
}
...
uint64_t is_valid_segment_write(uint64_t vaddr) {
  if (is_data_address(current_context, vaddr)) {
    ...
    return 1;
  } else if (is_valid_mapping_write(vaddr))
    return 1;
  else
    return 0;
}
...
// -----------------------------------------------------------------
// ---------- PAGE CACHE AND MEMORY MAPPINGS IMPLEMENTATION ---------
// -----------------------------------------------------------------

void init_page_cache() {
  if (cache_frames == (uint64_t*) 0) {
    cache_frames = zmalloc(NUMBEROFCACHEFRAMES * PAGESIZE);
    page_cache_entries = (uint64_t*) 0;
    next_cache_frame_index = 0;
    cache_frames_allocated = 0;
    page_cache_clock = 1;
  }
}

void init_file_identity_table() {
  if (fd_file_ids == (uint64_t*) 0) {
    fd_file_ids = zmalloc(MAX_TRACKED_FILE_DESCRIPTORS * sizeof(uint64_t));
    file_id_names = zmalloc(MAX_TRACKED_FILES * sizeof(uint64_t));
    next_file_id = 1;
  }
}
...
void register_file_identity(uint64_t fd, char* filename) {
  ...
  if (fd >= MAX_TRACKED_FILE_DESCRIPTORS)
    return;
  ...
  while (file_id < next_file_id) {
    if (*(file_id_names + file_id) != 0)
      if (string_compare((char*) *(file_id_names + file_id), filename)) {
        *(fd_file_ids + fd) = file_id;
        return;
      }
...
void touch_page_cache_entry(uint64_t* entry) {
  if (entry != (uint64_t*) 0) {
    set_page_cache_last_used(entry, page_cache_clock);
    page_cache_clock = page_cache_clock + 1;
  }
}
...
uint64_t* evict_page_cache_entry() {
  ...
  uint64_t* victim;
  uint64_t* prev;
  uint64_t* victim_prev;
  uint64_t oldest;
  uint64_t* frame;

  entry = page_cache_entries;
  victim = (uint64_t*) 0;
  prev = (uint64_t*) 0;
  ...
  while (entry != (uint64_t*) 0) {
    if (get_page_cache_refcount(entry) == 0)
      if (victim == (uint64_t*) 0 || get_page_cache_last_used(entry) < oldest) {
        victim = entry;
        victim_prev = prev;
        oldest = get_page_cache_last_used(entry);
      }

    prev = entry;
    entry = get_page_cache_next(entry);
  }
  ...
  frame = get_page_cache_frame(victim);

  if (victim_prev != (uint64_t*) 0)
    set_page_cache_next(victim_prev, get_page_cache_next(victim));
  else
    page_cache_entries = get_page_cache_next(victim);
...
uint64_t* allocate_cache_frame() {
  ...
  if (cache_frames_allocated >= NUMBEROFCACHEFRAMES)
    return (uint64_t*) 0;

  frame = cache_frames + (next_cache_frame_index * (PAGESIZE / WORDSIZE));
  next_cache_frame_index = next_cache_frame_index + 1;
  cache_frames_allocated = cache_frames_allocated + 1;
...
uint64_t* find_page_cache_entry(uint64_t fd, uint64_t file_page) {
  ...
  file_id = get_file_id_for_fd(fd);
  entry = page_cache_entries;

  while (entry != (uint64_t*) 0) {
    if (get_page_cache_file_id(entry) == file_id)
      if (get_page_cache_file_page(entry) == file_page) {
...
uint64_t* find_or_create_cache_entry(uint64_t fd, uint64_t file_page) {
  ...
  frame = allocate_cache_frame();
  if (frame == (uint64_t*) 0)
    frame = evict_page_cache_entry();

  if (frame == (uint64_t*) 0)
    return (uint64_t*) 0;

  entry = smalloc(PAGE_CACHE_ENTRY_SIZE * sizeof(uint64_t));
  file_id = get_file_id_for_fd(fd);

  set_page_cache_next(entry, page_cache_entries);
  set_page_cache_file_id(entry, file_id);
  set_page_cache_file_page(entry, file_page);
  set_page_cache_frame(entry, frame);
  set_page_cache_dirty(entry, 0);
  set_page_cache_refcount(entry, 0);
  set_page_cache_fd(entry, fd);
  touch_page_cache_entry(entry);
...
void increment_page_cache_refcount(uint64_t* entry) {
  if (entry != (uint64_t*) 0)
    set_page_cache_refcount(entry, get_page_cache_refcount(entry) + 1);
}
    ...
    if (get_page_cache_refcount(entry) > 0) {
      ...
      if (get_page_cache_refcount(entry) == 0)
        if (get_page_cache_dirty(entry)) {
          load_file_page_into_cache_frame(get_page_cache_fd(entry), get_page_cache_file_page(entry), get_page_cache_frame(entry));
          set_page_cache_dirty(entry, 0);
        }
    }
}
...
uint64_t* create_mapping_entry(uint64_t addr, uint64_t orig_length, uint64_t rounded_length,
  ...
  mapping = smalloc(MAPPING_ENTRY_SIZE * sizeof(uint64_t));

  set_mapping_next(mapping, (uint64_t*) 0);
  set_mapping_addr(mapping, addr);
  set_mapping_original_length(mapping, orig_length);
  set_mapping_rounded_length(mapping, rounded_length);
  set_mapping_prot(mapping, prot);
  set_mapping_fd(mapping, fd);
  set_mapping_offset(mapping, offset);
  set_mapping_flags(mapping, MAPPING_ACTIVE);
...
uint64_t* find_mapping(uint64_t* context, uint64_t addr) {
  ...
  mapping = get_mappings_head(context);

  while (mapping != (uint64_t*) 0) {
    if (get_mapping_addr(mapping) == addr)
      if (get_mapping_flags(mapping) & MAPPING_ACTIVE)
        return mapping;
...
uint64_t is_address_in_mapping(uint64_t* mapping, uint64_t vaddr) {
  ...
  end = start + get_mapping_rounded_length(mapping);
...
uint64_t is_mmap_range_free(uint64_t* context, uint64_t addr, uint64_t length) {
  ...
  if (do_ranges_overlap(addr, length, get_code_seg_start(context), get_code_seg_size(context)))
    return 0;

  if (do_ranges_overlap(addr, length, get_data_seg_start(context), get_data_seg_size(context)))
    return 0;

  if (do_ranges_overlap(addr, length, get_heap_seg_start(context), get_program_break(context) - get_heap_seg_start(context)))
    return 0;
  ...
  while (mapping != (uint64_t*) 0) {
    if (get_mapping_flags(mapping) & MAPPING_ACTIVE)
      if (do_ranges_overlap(addr, length, get_mapping_addr(mapping), get_mapping_rounded_length(mapping)))
        return 0;
    mapping = get_mapping_next(mapping);
  }
  ...
  page_addr = addr;
  while (page_addr < end) {
    if (is_virtual_address_mapped(get_pt(context), page_addr))
      return 0;
...
uint64_t choose_mmap_address(uint64_t* context, uint64_t length) {
  ...
  addr = round_up(get_program_break(context) + PAGESIZE, PAGESIZE);
  stack_start = *(get_regs(context) + REG_SP);

  while (addr + length > addr) {
    if (addr + length >= stack_start)
      return 0;
...
void add_mapping_to_context(uint64_t* context, uint64_t* mapping) {
  set_mapping_next(mapping, get_mappings_head(context));
  set_mappings_head(context, mapping);
}
...
void remove_mapping_from_context(uint64_t* context, uint64_t addr) {
  ...
  while (mapping != (uint64_t*) 0) {
    if (get_mapping_addr(mapping) == addr) {
      if (prev != (uint64_t*) 0)
        set_mapping_next(prev, get_mapping_next(mapping));
      else
...
uint64_t is_address_in_any_mapping(uint64_t* context, uint64_t vaddr) {
  ...
  mapping = get_mappings_head(context);
  while (mapping != (uint64_t*) 0) {
    if (is_address_in_mapping(mapping, vaddr))
      return 1;
    mapping = get_mapping_next(mapping);
  }
  return 0;
...
uint64_t is_valid_mapping_read(uint64_t vaddr) {
  ...
  if (mapping != (uint64_t*) 0)
    if (is_mapping_readable(get_mapping_prot(mapping)))
      return 1;
...
uint64_t is_valid_mapping_write(uint64_t vaddr) {
  ...
  mapping = find_mapping_for_address(current_context, vaddr);
  if (mapping != (uint64_t*) 0)
    if (is_mapping_writable(get_mapping_prot(mapping)))
      return 1;
...
void mark_mapping_page_dirty(uint64_t vaddr) {
  ...
  mapping = find_mapping_for_address(current_context, vaddr);
  if (mapping != (uint64_t*) 0) {
    file_page = get_mapping_offset(mapping) / PAGESIZE;
    file_page = file_page + ((vaddr - get_mapping_addr(mapping)) / PAGESIZE);
    cache_entry = find_page_cache_entry(get_mapping_fd(mapping), file_page);
    mark_page_cache_entry_dirty(cache_entry);
  }
}
...
void inherit_mappings_on_fork(uint64_t* parent, uint64_t* child) {
  ...
  while (mapping != (uint64_t*) 0) {
    ...
    for (i = 0; i < page_count; i = i + 1) {
      ...
      vaddr_current = get_mapping_addr(mapping) + (i * PAGESIZE);
...
void implement_mmap(uint64_t* context) {
  ...
  if (is_mapping_prot_valid(prot) == 0) {
    *(get_regs(context) + REG_A0) = (uint64_t)-1;
    set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
    return;
  }
  ...
  for (i = 0; i < page_count; i = i + 1) {
    ...
    if (cache_entry != (uint64_t*) 0) {
      if (get_page_cache_dirty(cache_entry)) {
        ...
        if (bytes_to_write > PAGESIZE)
          bytes_to_write = PAGESIZE;
        if (write_cache_frame_to_file(get_mapping_fd(mapping), file_page,
                                      (uint64_t*) get_page_cache_frame(cache_entry), bytes_to_write) == 0) {
          *(get_regs(context) + REG_A0) = (uint64_t)-1;
          set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
          return;
        }
